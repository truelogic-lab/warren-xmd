...
// Tracks message IDs the bot has sent. When `fromMe` messages arrive,
// if the ID is in this set, it's the bot's own reply echoing back — ignore.
// If not, it's the user typing in self-chat — process as a command.
const sentMessageIds = new Map();
const SENT_ID_TTL = 20 * 60 * 1000; // 20 minutes
...
export async function handleMessage(sock, m, plugins) {
  try {
    ...
    // ─── SELF-CHAT ECHO FILTER ───
    // If message is fromMe, we need to distinguish:
    //   (a) The user typing in self-chat → process as command
    //   (b) The bot's own reply echoing back → ignore
    ...
    let sender;
    if (m.key.fromMe) {
      // Self-chat command — attribute to the bot's own number
      sender = botJid;
    } else if (isGroup) {
