...
const SENT_ID_TTL = 20 * 60 * 1000;

setInterval(() => {
  const cutoff = Date.now() - SENT_ID_TTL;
...
export function trackSentMessage(id) {
  if (id) sentMessageIds.set(id, Date.now());
}
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (!plugin || !plugin.triggers) {
          console.warn(`⚠️ ${cat}/${file}: missing triggers`);
          continue;
        }

        // Validation — code must be a function
        if (typeof plugin.code !== 'function') {
          console.warn(`⚠️ ${cat}/${file}: missing code function`);
          continue;
        }

        if (typeof plugin.triggers === 'string') {
          plugin.triggers = [plugin.triggers];
        }

        if (!Array.isArray(plugin.triggers)) {
          console.warn(`⚠️ ${cat}/${file}: triggers must be string or array, skipping`);
          continue;
        }

        plugin.triggers = plugin.triggers.map(t => String(t).toLowerCase());
...
export async function getOwners() {
  ...
  if (list.length === 0 && settings.ownerNumber) {
    list.push(settings.ownerNumber);
  }
...
export async function handleMessage(sock, m, plugins) {
  try {
    ...
    // ─── SELF-CHAT ECHO FILTER ───
    if (m.key.fromMe && wasSentByBot(msgId)) {
      return;
    }
    ...
    // React so the sender knows the bot received it
    try {
    ...
    try {
      if (typeof plugin.code !== 'function') {
        console.error(`[${command}] plugin has no valid code function`);
        return;
      }

      await plugin.code(ctx);
    } catch (err) {
      ...
      try {
        ...
        console.error(`[${command}] failed to send error reply:`, sendError.message);
      }
    }
  } catch (err) {
...
export async function reloadPlugin(filePath) {
  try {
    ...
    if (mod.default?.triggers) {
      ...
      if (!Array.isArray(mod.default.triggers)) {
        console.warn(`⚠️ reloadPlugin: triggers must be array, skipping`);
        return null;
      }
      if (typeof mod.default.code !== 'function') {
        console.warn(`⚠️ reloadPlugin: missing code function`);
        return null;
      }
      mod.default.triggers = mod.default.triggers.map(t => String(t).toLowerCase());
