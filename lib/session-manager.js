...
export async function createSession(phoneNumber, plugins = null) {
  ...
  sock.sendMessage = async (jid, content, options = {}) => {
    return enqueue(phoneNumber, async () => {
      try {
        ...
        // Track sent ID so self-chat echoes get filtered in handler.js
        if (result?.key?.id) {
  ...
  const connectionHandler = async ({ connection, lastDisconnect }) => {
    if (connection === 'close') {
      ...
      // WhatsApp asks the client to reconnect immediately.
      // Do NOT wait — resume the pairing handshake fast.
      if (code === 515) {
      ...
      // ─── 401 AFTER 515 DURING PAIRING ───
      // WhatsApp sometimes returns 401 after 515 during initial pairing.
      // Treat it as transient — retry a few times before giving up.
      ...
      // ─── TERMINAL (genuine logout / corruption) ───
      ...
      if (botJid) {
        ...
        try {
          ...
          // Mark the welcome message as sent by bot so it doesn't echo back
          if (sent?.key?.id) {
