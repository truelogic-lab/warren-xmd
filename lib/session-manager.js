...
export async function createSession(phoneNumber, plugins = null) {
  // If a session exists but socket is dead, destroy it first
  ...
  // Validate auth state is fully loaded
  if (!state?.creds) {
    throw new Error(`Auth state not ready for ${phoneNumber}`);
  }

  // @zentrix/baileys handles version internally — no version parameter needed
  ...
  const connectionHandler = async ({ connection, lastDisconnect }) => {
    if (connection === 'close') {
      ...
      // Destroy old socket BEFORE scheduling reconnect
      await destroySocket(phoneNumber);
