...
export async function createSession(phoneNumber, plugins = null) {
  // MULTI-TENANT SAFE:
  // - Different numbers can pair simultaneously (each gets its own socket)
  // - Same number with a LIVE socket is reused
  // - Same number with a DEAD socket is destroyed and recreated
  if (sessions.has(phoneNumber)) {
    const existing = sessions.get(phoneNumber);
    // If the socket is alive (has a user OR is actively mid-pairing), reuse
    if (existing.sock?.user || existing.sock?.authState?.creds) {
      return existing;
    }
    // Genuinely dead — clean it up before creating fresh
