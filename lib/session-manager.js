...
export async function createSession(phoneNumber, plugins = null) {
  ...
  const connectionHandler = async ({ connection, lastDisconnect }) => {
    if (connection === 'close') {
      ...
      // WhatsApp protocol asks client to reconnect immediately.
      // Do NOT wait — the pending pairing handshake must resume fast.
      if (code === 515) {
      ...
      // ─── 401 RIGHT AFTER 515 ───
      // During initial pairing, WhatsApp sometimes returns 401 after a 515.
      // That is NOT a permanent logout — the pairing just needs one more try.
      if (code === 401 && flags.received515) {
        console.log('⚠️ Session ' + phoneNumber + ' got 401 after 515 — treating as transient, retrying');
